...
| **슬라이드쇼** | Ken Burns(팬/줌) 효과를 **순서대로 순환**, 전환 효과 5종(크로스페이드/슬라이드/줌/밀어올리기/혼합), 간격 2~60초 |
| **콜라주** | 2~9장을 **창의적인 다양한 레이아웃**(장수별 다수 변형)으로 배치, 사진 앱 PLAY처럼 매 장면 새 사진/배치로 진행 |
| **위젯 오버레이** | 시계·날짜, 날씨(WeatherKit)를 액자 위에 **선택적으로** 표시 |
| **그리드 갤러리** | 전체 사진을 격자로 스크롤, 탭하면 전체화면 |
| **배경음악** | 음악 파일(.mp3/.m4a 등)을 가져와 슬라이드쇼 중 반복 재생, 볼륨 조절 |
| **2단 이미지 캐시 + 미리 받기** | 메모리+디스크 캐시 및 다음 사진 prefetch로 매끄러운 전환, 재다운로드 방지 |
| **화면 크기 맞춤 다운로드** | 큰 원본 대신 iPad 화면 해상도에 맞춰 받아 속도·메모리 최적화 |
